...
class CustomPolicy(object):
    ...
    def gradient_step(self, grad, step_size):
        ...
        self.weights = self.weights + step_size*np.reshape(grad,[-1,1])
...
def compute_rbf_centers(state_high, state_low, no_rbf):
    pos_state = np.linspace(state_low[0], state_high[0],no_rbf+2)
    vel_state = np.linspace(state_low[1], state_high[1],no_rbf+2)
